...
describe("#knowledgeBase/createCapcityStandard",function(){
    ...
    it('create capcitystandard successfully',async()=>{
        ...
        try{
            await page.goto("https://web-beta.blacklake.cn/knowledgeManagement/productivityStandards",{
                timeout:100000
            });
            await page.waitForSelector(".ant-breadcrumb-link",{timeout:100000});
            
            //获取产能标准列表页面包屑
            const breakCrumbName = await page.evaluate(() =>
                document.getElementsByClassName("ant-breadcrumb-link")[0].innerText
            )
        ...
        try{
            ...
            let myNumber=Array();
            myNumber[0]="0";
            myNumber[1]="100000000000";
            myNumber[2]="9999999999.99999";
            myNumber[3]="100";
           for (x in myNumber){
                //输入产能标准数值
                try{
                    ...
                    if(x==0){
                        ...
                        const rēsult = await page.$eval('.ant-form-explain',x=>x.innerText);
                        console.log('产能标准输入100000000000:',rēsult);
                        assert.equal(rēsult, '数字必需小于等于99999999999','rēsult = 数字必需小于等于99999999999');
                    } else if(x==2){
                        const rēsult = await page.$eval('.ant-form-explain',x=>x.innerText);
                        console.log('产能标准输入四位小数9999999999.9999:',rēsult);
                        assert.equal(rēsult, '小数点后最多保留3位数字','rēsult = 小数点后最多保留3位数字');
                    }
                } catch(e){
