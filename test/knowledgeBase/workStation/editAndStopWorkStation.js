...
describe('#knowledgeBase/editAndDelectCustomer', function () {
	...
	it('创建单位', async() => {
        ...
        //创建一个单位（默认会显示在第一行）
        const workStation = await precondition.createWorkStation(page);
        console.log('work station is',workStation);
        //点击编辑按钮
        await event.clickElement(page, '.anticon.anticon-ellipsis', 0);
        await event.clickElement(page, '.actionButtonContainer .anticon.anticon-edit', 0);
        await page.screenshot({ path: `images/${filename}.png` });

        // //wait for 取消按钮 appear
        // await page.waitForSelector('.anticon.anticon-edit');
